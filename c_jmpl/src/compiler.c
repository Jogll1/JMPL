...
static void initCompiler(Parser* parser, Compiler* compiler, FunctionType type) {
    ...
    if (type != TYPE_SCRIPT) {
        parser->currentCompiler->function->name = copyString(parser->gc, parser->strings, parser->previous.start, parser->previous.length);
    }
...
static ObjFunction* endCompiler(Parser* parser) {
    ...
    if (!parser->hadError) {
        disassembleChunk(currentChunk(parser), function->name != NULL ? (const char*)function->name->chars : "<script>");
    }
#endif
 ...
 */
static void functionWrapper(Parser* parser, FunctionType type, void (*f)(Parser*)) {
    Compiler compiler;
...
static void binary(Parser* parser, bool canAssign) {
    ...
    switch (operatorType) {
        ...
        case TOKEN_EQUAL:         emitByte(parser, OP_EQUAL);          break;
 ...
 */
static void implMult(Parser* parser, bool canAssign) {
...
static void setBuilder(Parser* parser) {
    ...
    parser->currentCompiler->function->name = copyString(parser->gc, parser->strings, "@setb", 5);
    ...
    *parser = initialParser;
    ...
    emitByte(parser, OP_SET_INSERT); // This seems to update without OP_SET_LOCAL - pointer fault?
    emitByte(parser, OP_POP);
        ...
        if (check(parser, TOKEN_ELLIPSIS)) {
            ...
            if (match(parser, TOKEN_COMMA)) {
                ...
                if (check(parser, TOKEN_ELLIPSIS)) {
                    ...
                    emitByte(parser, OP_SET_INSERT_2);

                    while (match(parser, TOKEN_COMMA)) {
                        expression(parser, true);

                        emitByte(parser, OP_SET_INSERT);
                    }
                }
            } else {
                // Singleton set
...
static void quantifier(Parser* parser) {
    // Set anonymous function name and to implicit return
    parser->currentCompiler->function->name = copyString(parser->gc, parser->strings, "@quan", 5);
    parser->currentCompiler->implicitReturn = true;
    ...
    // Predicate
    consume(parser, TOKEN_COMMA, "Expected comma after generator of quantifier");
    expression(parser, false);
 ...
 */
static void quantAnon(Parser* parser, bool canAssign) {
...
static void markCompilerRoots(struct GC* gc, void* arg) {
    ...
    if (parser->prevMarkRoots) {
        parser->prevMarkRoots(gc, parser->prevMarkRootsArgs);
    }
}
...
static void setupGC(Parser* parser, GC* gc, Table* strings) {
    ...
    parser->prevMarkRootsArgs = gc->markRootsArgs;
    parser->prevFixWeak = gc->fixWeak;
    parser->prevFixWeakArg = gc->fixWeakArg;

    gc->markRoots = markCompilerRoots;
    gc->markRootsArgs = parser;
...
static void restoreGC(Parser* parser) {
    ...
    gc->markRootsArgs = parser->prevMarkRootsArgs;
    gc->fixWeak = parser->prevFixWeak;
    gc->fixWeakArg = parser->prevFixWeakArg;
}

ObjFunction* compile(const unsigned char* source, GC* gc, Table* strings) {
    Parser parser;
